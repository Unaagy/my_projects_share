...
#Создаем DAG
dag_create = DAG(
    'detealed_file_processing_pipeline',            #Название пайплайна
...
#Генерируется один раз за запуск скрипта — это "дата run DAG"
report_period = '{{ ds }}'

#Пути к исходному и выходному файлам, а также к базе данных
input_file = '/opt/airflow/dags/YM_demo_attendance_for_six_months.csv'       #загруженный файл с Яндекс Метрики
output_file = '/opt/airflow/dags/clean_demo_data.csv'                        #файл с очищенными данными
output_file2 = '/opt/airflow/dags/data_indicators.csv'
db_path = '/opt/airflow/dags/visits.db'                                      #файл с таблицей в БД SQL
...
check_files_excistence_task = PythonOperator(
    task_id='check_file_excistence',
    python_callable=check_file_existence,   #Функция
    op_args=[input_file],                   #Параметры для функции
    dag=dag_create,
)

#Task 2: Проверка наличия столбцов в исходном файле
validate_columns_task = PythonOperator(
    task_id='validate_columns',
    python_callable=validate_columns,   #Функция
    op_args=[input_file],                   #Параметры для функции
    dag=dag_create,
)

#Task 3: Проверка качества данных в исходном файле
validate_data_quality_task = PythonOperator(
    task_id='validate_data_quality',
    python_callable=validate_data_quality,   #Функция
    op_args=[input_file],                   #Параметры для функции
    dag=dag_create,
)

#Task 4: Предобработка данных (очистка, фильтрация) в исходном файле
preprocess_data_task = PythonOperator(
    task_id='preprocess_data',
    python_callable=preprocess_data,   #Функция
    op_args=[input_file, output_file, report_period],  #Параметры для функции
    dag=dag_create,
)

#Task 5: Проверка наличия обработанных данных
check_processed_file_task = PythonOperator(
    task_id='check_processed_file',
    python_callable=check_processed_file,   #Функция
    op_args=[output_file],                  #Параметры для функции
    dag=dag_create,
)

#Task 6: Создание дополнительных метрик оценки данных
added_metrics_task = PythonOperator(
    task_id='added_metrics',
    python_callable=added_metrics,          #Функция
    op_args=[output_file, output_file2, report_period], #Параметры для функции
    dag=dag_create,
)

#Task 7: Инициализация базы данных SQLite
init_db_task = PythonOperator(
    task_id='init_db',
    python_callable=init_db,                #Функция
    op_args=[db_path],                      #Параметры для функции
    dag=dag_create,
)

#Task 8: Загрузка обработанных данных в таблицы базы данных SQLite
load_data_to_db_task = PythonOperator(
    task_id='load_data_to_db',
    python_callable=load_data_to_db,          #Функция
    op_args=[db_path, output_file, output_file2, report_period],  #Параметры для функции
    dag=dag_create,
)

#Task 9: Проверка данных в БД
check_table_data_task = PythonOperator(
    task_id='check_table_data',
    python_callable=check_table_data,          #Функция
    op_args=[db_path],                         #Параметры для функции
    dag=dag_create,
)

#Task 10: Выгрузка файлов 'visits_data' и 'attend_summary' в Excel для Datalens
export_for_datalens_task = PythonOperator(
    task_id='export_for_datalens',
    python_callable=export_for_datalens,
    op_args=[db_path, datalens_export_path],
    dag=dag_create,
)

#Task 11: Проверка выгруженных файл на наличие
check_datalens_export_task = PythonOperator(
    task_id='check_datalens_export',
    python_callable=check_datalens_export,
    op_args=[datalens_export_path],
    dag=dag_create,
)

#Task 12: Создание пустышки для начала и конца пайплайна
start_task = PythonOperator(
    task_id='start',
    python_callable=lambda: print("Пайплайн стартовал"),  #Псевдо-функция для старта
    dag=dag_create,
)

end_task = PythonOperator(
    task_id='end',
    python_callable=lambda: print("Пайплайн завершен"),  #Псевдо-функция для завершения
    dag=dag_create,
)

#Последовательность выполнения задач
start_task >> check_files_excistence_task >> validate_columns_task >> validate_data_quality_task \
>> preprocess_data_task >> check_processed_file_task >> added_metrics_task >> init_db_task \
>> load_data_to_db_task >>check_table_data_task >> export_for_datalens_task \
>> check_datalens_export_task >> end_task